...
<html lang="zxx">
   <head>
      ...
      <meta name="description" content="Login to Study Wander Account" />
      <meta name="keywords" content="login, study, wander, account" />
      ...
      <div class="splash-auth-wrapper">
         <div class="splash-auth-box">
            <div class="splash-auth-content">
               <form method="post">
                  <a href="./" class="splash-auth-logo">
                     <img src="assets/logo2.png" alt="" />
                  </a>
                  <!-- <h2>Hello, Welcome to <span>Admed Admin</span></h2> -->
                  ...
                  <div class="splash-auth-btn">
                     <button type="submit" name="submit" class="splash-btn splash-login-memeber w-100" id="toastr-4">Login</button>
                     <!-- <a href="" class="splash-btn splash-login-member w-100" type="submit" name="submit">Login</a> -->
                  </div>
                  <p class="splash-register-text">username=admin, password= admin123</p>
               </form>
            </div>
         </div>
      </div>
...
if (isset($_REQUEST['submit'])) {
    ...
    if ($row = mysqli_fetch_array($query)) {
        ...
        if ($user == $userdb && $pass == $passwordb) {
            $_SESSION['loggedin'] = "success"; 
            // $_SESSION['email'] = $_SESSION['email'];
            echo "<script>alert('Login Successfully');window.location.href='./';</script>";
          
        }elseif($pass!==$passwordb){
          echo "<script>
                        iziToast.error({
                            title: 'Error',
   ...
   // echo "checking". $_SESSION['loggedin'] ;
?>
